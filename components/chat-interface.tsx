...
export default function ChatInterface() {
  ...
  // Load messages when a contact is selected and mark as read
  useEffect(() => {
    ...
    const markAsRead = async () => {
      try {
        ...
        // Update local contacts state to reflect zero unread count
  ...
  // Auto-scroll to bottom with mobile header preservation
  useEffect(() => {
  ...
  const handleInputChange = useCallback(
    (e: React.ChangeEvent<HTMLTextAreaElement>) => {
      ...
      // Immediate typing indicators without debounce for better responsiveness
  ...
  const handleSendMessage = useCallback(
    async (e: React.FormEvent) => {
      ...
      // Keep focus on mobile to prevent keyboard flickering
      ...
      try {
        ...
        // Maintain focus on mobile without delay
        if (isMobile && messageInputRef.current) {
          // Use requestAnimationFrame to ensure smooth focus retention
          requestAnimationFrame(() => {
          ...
          // Desktop behavior with small delay
          setTimeout(() => {
            messageInputRef.current?.focus()
          }, 50)
        }
        ...
        >
          {selectedContact || selectedGroup ? (
            <>
              ...
              {/* WhatsApp-style Message Input with Emoji Picker */}
              <div
